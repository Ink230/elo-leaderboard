...
namespace NetCoreServer
{
    ...
    public class HttpRequest
    {
        ...
        /// <summary>
        /// Directly acquire the X-Real-IP header set by nginx
        /// </summary>
            ...
            for (int i = _cacheSize; i < (int)_cache.Size; ++i)
            {
                ...
                if ((_cache[i + 0] == '\r') && (_cache[i + 1] == '\n') && (_cache[i + 2] == '\r') && (_cache[i + 3] == '\n'))
                {
                    ...
                    while ((index < (int)_cache.Size) && (index < i))
                    {
                        ...
                        //Assign X-Real-IP
                        if (string.Compare(headerName, "X-Real-IP", StringComparison.Ordinal) == 0)
                        {
                            _XRealIP = headerValue;
                        }
